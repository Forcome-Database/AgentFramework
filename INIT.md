...
这条漂移风险留到阶段 7 报告，由用户决定是否把 `CLAUDE.md` 的技术细节合并进 `AGENTS.md`。想让它自动化，用 `refactor-legacy`。那里的 `legacy/doc-fork` 会做这件事——它会判断哪两条重复、某条该进哪一节、两条是不是在互相矛盾，并把每一个「没有逐字保留原文」的决定写进决策日志 `.agents/refactor-decisions.md`。
