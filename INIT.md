...
- 框架根目录：本文件所在的目录。作为 Claude Code 插件安装时，它等于 `${CLAUDE_PLUGIN_ROOT}`；直接使用仓库时，它就是仓库根目录。下文所有 `reference/`、`templates/`、`scripts/` 路径均相对于框架根目录。
...
退出条件：五道门全过，且 `check-docs.mjs` 返回 0。
