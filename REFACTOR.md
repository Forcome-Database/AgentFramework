...
**记录 `check-docs.mjs` 的基线：**跑 `node scripts/check-docs.mjs <目标项目根目录>`，把它输出的每一条错误**按类型**记下，下文称**基线错误类型集**。返回 0 则该集为空。

`check-docs.mjs` 只会报三类错误，按下表归类：

| 类型 | 错误信息的特征 |
| --- | --- |
| `transclusion` | 含「`CLAUDE.md` 应恰为一行 transclusion」或「`CLAUDE.md` 唯一一行应为」或「`CLAUDE.md` 不存在」 |
| `行数预算` | 含「超出 300 行预算」 |
| `死链` | 含「死链」 |
| `缺失` | 含「`AGENTS.md` 不存在」 |

**记类型，不要记原样字符串。**因为 `check-docs.mjs` 把行数嵌在错误信息里（`实际有 204 行非空内容`、`AGENTS.md 有 350 行`），而 `legacy/memory-bloat` 的**唯一工作就是把行数改小**。按字符串比对时，一次**成功**的搬迁会把 `实际有 204 行` 变成 `实际有 101 行`——后者不在基线里，会被判成「新错误」并触发回滚。**那个块干得越对，越会被自己回滚。**
...
1. **路径存在门。** 改动后的文档中，每一条肯定句所断言的位置路径，必须 `test -e` 通过。
   ...
   **被 `.gitignore` 匹配的路径豁免本门。**因为它们按定义就不在仓库里：`.env` 是运行时文件，仓库里只有 `.env.example`。这一条不能省 —— `legacy/memory-bloat` 会把约束文件的整章搬进 `docs/`，被搬走的章节里完全可能有「环境变量统一从 `.env` 读取」这类肯定句，那个新文件会进入改动文件清单，然后被本门判为死链。
   ...
   **`%/` 不能省。**`git check-ignore` 对一个带尾部斜杠且不存在的路径会给出假阳性，返回 0。不剥掉它，本门会把每一个不存在的目录都判为「已被 gitignore，豁免」，这道门就废了（证据：`docs/pitfalls.md` 第 10 条）。
...
4. **不倒退门。** 重跑 `node scripts/check-docs.mjs <目标项目根目录>`，把输出的错误**按阶段 2 那张表归类**，与**基线错误类型集**比对。

   **判据：不得出现基线类型集里没有的新类型。**新类型即阻断，必须修掉或回滚引入它的那个块。

   **按类型比，不要按字符串比。**同一类型的错误，其中的数字变小是**本轮工作的成果**，不是新错误：`memory-bloat` 搬走章节后，`行数预算` 类的错误会从「350 行」变成「280 行」甚至消失，`transclusion` 类的会从「实际有 204 行」变成「实际有 101 行」。按字符串比对会把这些成果判成新错误并回滚掉它们。

   基线类型集里已有、本轮未消除的类型**不阻断**——但每一个都必须能对应到【报告】组中的某一条，**或**阶段 2 的已忽略清单中的某一条。两处都对应不上，就说明有一类腐烂既没被自动整理、也没被报告、也没被用户主动忽略，那是漏检，同样阻断。
...
5. **互斥门。** 检查扫描集中是否有 `exclusive-with` 非 `null` 的块。
   ...
   当前规则库下，扫描集中**没有互斥块**——只有 `legacy/` 的 5 个块带 `Legacy Scan` 字段，它们的 `exclusive-with` 全是 `null`。（扫描集本身不为空，它恰恰就是这 5 个块。）但 `reference/rule-block-spec.md` 明确允许任何块同时拥有 `Verification` 与 `Legacy Scan`，所以一旦某个互斥块（如 `core/production-migration-required`）被补上 `Legacy Scan`，它就会进入扫描集。**那时这道门必须接住它**：互斥对在重构场景下如何取舍是未定义的，遇到即停下来告诉用户，不要自行选一个。
