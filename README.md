...
| 规则块 | 档位 | 它要做的事需要理解内容的含义吗 |
| --- | --- | --- |
| `doc-index-rot` | `自动` | 否 —— 从文件清单机械生成 `docs/index.md` |
| `memory-bloat` | `自动` | 否 —— 按二级标题整章搬进 `docs/`，原位留指针 |
| `doc-fork` | `自动` | 否 —— **append-only by section**：整章追加，永不去重、永不重排、永不消解矛盾 |
| `orphan-abstraction` | `报告` | **是** —— 空目录与「正在重建的活目录」在文件系统上同形 |
| `vendored-knowledge` | `报告` | **是** —— 无法自动区分哪些行是项目特有配置 |

**「代码永远只报告」是这条线的推论，不是这条线本身。**

### `doc-fork` 为什么能自动：把动作缩窄，而不是加一个档位

`doc-fork` 一度被压成报告档，理由是「合并需要判断：哪两条重复？某条该进哪一节？两条是不是在互相矛盾？」——这三个判断确实是语义的。

但**它们可以直接不做**。

`doc-fork` 的目标不是「产出一份干净的合并文件」，是「**消灭分叉**」。分叉的危害是 agent 会读到过期的那一份（`anti-patterns.md` 第 1 条）。所以只要两份文件变成一份、一行内容都没丢，目标就达到了——去重和消解矛盾是**额外的**，不是必需的。

于是动作被重新定义为 **append-only by section**：按二级标题整章追加。重复条目和矛盾指令会**并排出现在同一个文件里**——而它们本来就在，只是分散在两个文件里。追加之后没有「过期的那一份」了，两种说法都在眼前，优先级由 `meta/constraint-precedence` 处理。

**这个动作一个语义判断都不需要。它是机械的。**

判据变了，档位就跟着变了——**不需要新增一个「授权后自动」的第三档**。当一个动作看起来「需要授权」时，先问：是它本质上不可验证，还是**我把它定义得太宽了**？加一个 enum 值是最容易的动作，也最容易掩盖真正的设计错误。

安全性由**内容保全核对**兜底，它覆盖 100% 的非标题行（不只是条目行），搜索范围是 `AGENTS.md` 加 `docs/`——因为「内容保全」的语义是「没有任何一行被删掉」，不是「全部进了 `AGENTS.md`」。任何一行丢失即整体回滚。
