...
| `报告` | 只列清单，不动文件 | 疑似废弃的空目录、约束文件里的第三方库知识 |

**代码永远只报告，不动手。**这是设计上限，由 `validate-rules.mjs` 强制：标为 `自动` 的块必须声明写作用域，且每项须落在文档白名单内。这个白名单不是一个正则，是 `isDocScope()` 函数，它守两个上限——类型（必须是 `.md` 文件，或 `docs/` 下的目录）与位置（必须是仓库内的相对路径；绝对路径、`~ $ %`、`..` 逃逸、glob 一律拒绝）。
...
**一个真实的排除条件：`legacy/doc-fork` 遇到非条目内容时转报告。** 当 `CLAUDE.md` 中含代码块、表格或散文段落时，这个块不自动合并，转入报告档，理由是它的内容保全核对只逐行比对条目，对这些内容完全失明。这不是理论顾虑：一个真实老项目的 `CLAUDE.md` 共 682 行，其中 328 行是条目、69 行是标题、96 行是空行，剩下 **189 行（占全文 28%）** 是架构图、表格与代码块——全部落在核对的盲区里。宁可把整块降级为报告，也不要交付一份对 28% 内容失明、却显示「核对通过」的自动合并。

### 六个阶段与不倒退门

扫描手册 `REFACTOR.md` 是六个阶段：前置（`AGENTS.md` 必须存在、工作区必须干净）、确定扫描集、扫描、分级呈现、执行【自动】组、自检。阶段 5「自检」是**五道门**：路径存在门、占位符门、溯源门、互斥门，加一道**不倒退门**——不要求 `node scripts/check-docs.mjs` 返回 0，只要求它不比阶段 2 记录的基线更差。

这条设计是必需的，不是将就：`legacy/doc-fork` 会正确地拒绝合并含代码块或表格的 `CLAUDE.md`，此时 `CLAUDE.md` 保持多行，`check-docs.mjs` 会永远报错。若不倒退门要求返回 0，手册会在这里死锁——老项目文档已经腐烂正是它存在的理由，不是异常。
