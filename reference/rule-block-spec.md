...
一度有过一个「可逆性」枚举（自动 / 报告），试图**提前静态地**判定一个动作安不安全。它在开发中翻改了四次 —— 不是想不清楚，是**这个问题在静态层面无解**：安不安全依赖具体项目，不依赖规则。少一个枚举，就少一处会分叉的事实、少一张要同步的 README 表格。

### 该判断的就判断，但必须交账

**自动整理不是「只做机械的、不需要理解内容的动作」——你手里有一个大模型，用它。**

`legacy/memory-bloat` 要逐章读内容，判断哪章是约束、哪章是踩坑日志、哪章是参考资料。`legacy/doc-fork` 要判断哪两条重复、某条该进哪一节、两条是不是在互相矛盾。这些判断模型做得到。

代价是：**每一个「没有逐字保留原文」的决定，必须写进 `.agents/refactor-decisions.md`，附原文与理由。**由各块 `Verification` 中的**零静默丢失核对**机器强制：原文的每一行，要么逐字存在于某个文件，要么出现在决策日志里，两者都不满足即阻断。
...
声明了 `作用域` 的块，其每一项须通过 `scripts/validate-rules.mjs` 导出的 `isDocScope(scope)`：
...
**作用域约束的是「写」的范围，不是「扫」的范围。** `Legacy Scan` 按定义只读 —— `REFACTOR.md` 阶段 1 禁止任何写操作 —— 所以它扫什么无关安全。
