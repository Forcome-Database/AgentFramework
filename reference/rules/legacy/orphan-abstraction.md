...
- 该目录的唯一子项是 `.gitkeep` 或 `.keep` —— 它是「这个空目录是被故意保留的」的标准标记，git 不追踪空目录，留下它正是为了让目录活下来。
...
- 不要只用 `git log` 判活 —— 因为它只读已提交历史，对工作区里未提交的删除完全视而不见：文件两年前提交、今天刚被 rm 掉但没提交，`git log -1 -- <dir>` 照样返回两年前（证据：`docs/pitfalls.md` 第 5 条）。
